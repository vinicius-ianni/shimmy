...
fn test_template_files_are_included_in_package() {
    ...
    assert!(!_dockerfile.is_empty(), "Dockerfile template should not be empty");
    assert!(!_compose.is_empty(), "docker-compose.yml template should not be empty");
    assert!(!_nginx.is_empty(), "nginx.conf template should not be empty");
    assert!(!_deployment.is_empty(), "deployment.yaml template should not be empty");
    assert!(!_service.is_empty(), "service.yaml template should not be empty");
    assert!(!_configmap.is_empty(), "configmap.yaml template should not be empty");
    assert!(!_railway.is_empty(), "railway.toml template should not be empty");
    assert!(!_fly.is_empty(), "fly.toml template should not be empty");
    assert!(!_fastapi_main.is_empty(), "FastAPI main.py template should not be empty");
    assert!(!_fastapi_requirements.is_empty(), "FastAPI requirements.txt template should not be empty");
    assert!(!_express_app.is_empty(), "Express app.js template should not be empty");
    assert!(!_express_package.is_empty(), "Express package.json template should not be empty");
}
...
fn test_template_content_validity() {
    ...
    assert!(dockerfile.contains("FROM"), "Dockerfile should contain FROM instruction");

    let compose = include_str!("../templates/docker/docker-compose.yml");
    assert!(compose.contains("version:") || compose.contains("services:"),
           "docker-compose.yml should contain version or services");

    let deployment = include_str!("../templates/kubernetes/deployment.yaml");
    assert!(deployment.contains("apiVersion:"), "deployment.yaml should contain apiVersion");

    let fastapi_main = include_str!("../templates/frameworks/fastapi/main.py");
    assert!(fastapi_main.contains("FastAPI"), "FastAPI template should reference FastAPI");

    let express_app = include_str!("../templates/frameworks/express/app.js");
    assert!(express_app.contains("express"), "Express template should reference express");
}