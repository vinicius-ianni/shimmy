...
mod vision_license_tests {
    ...
    async fn test_check_response_freshness_future_edge_case() {
        ...
        let result = VisionLicenseManagerTestWrapper::check_response_freshness(&date_header);
        assert!(result.is_ok(), "Time 30 seconds in future should be valid (clock skew tolerance)");
    }
    ...
    async fn test_check_response_freshness_future_invalid() {
        ...
        let result = VisionLicenseManagerTestWrapper::check_response_freshness(&date_header);
        assert!(result.is_err(), "Time 2 minutes in future should be invalid");
    ...
    async fn test_verify_response_signature_missing_signature_field() {
        ...
        let sig_header = r#"keyid="test", algorithm="ed25519", headers="(request-target) host date digest""#;
        ...
        let result = VisionLicenseManagerTestWrapper::verify_response_signature(
            sig_header, date_header, response_body
        );
        assert!(result.is_err(), "Missing signature field should return error");
    ...
    async fn test_verify_response_signature_malformed_header_no_quotes() {
        ...
        assert!(result.is_err(), "Malformed header (no quotes) should return error");
    ...
    async fn test_verify_response_signature_invalid_base64() {
        ...
        assert!(result.is_err(), "Invalid base64 signature should return error");
    ...
    async fn test_verify_response_signature_wrong_signature_length() {
        ...
        assert!(result.is_err(), "Wrong signature length should return error");
    ...
    fn test_vision_license_error_to_json_error_validation_failed() {
        ...
        assert_eq!(json["error"]["message"], "License validation failed: network timeout");
    }
    ...
    fn test_vision_license_error_to_json_error_feature_not_enabled() {
        ...
        assert_eq!(json["error"]["message"], "Vision feature not enabled for this license");
    }
    ...
    async fn test_vision_license_manager_new() {
        ...
        assert_eq!(usage.requests_today, 0, "New manager should have zero daily requests");
        assert_eq!(usage.requests_this_month, 0, "New manager should have zero monthly requests");
    }
    ...
    async fn test_record_usage_increments_counters() {
        ...
        let usage = manager.get_usage_stats().await;
        assert_eq!(usage.requests_today, 1, "Daily counter should be incremented");
        assert_eq!(usage.requests_this_month, 1, "Monthly counter should be incremented");
    }
    ...
    async fn test_record_usage_multiple_calls() {
        ...
        assert_eq!(usage.requests_this_month, 5, "Monthly counter should accumulate");
    }
    ...
    fn test_license_validation_serialization() {
        ...
        entitlements.insert("monthly_cap".to_string(), serde_json::Value::Number(1000.into()));
        
        let mut meta = HashMap::new();
        meta.insert("code".to_string(), serde_json::Value::String("VALID".to_string()));
    ...
    async fn test_check_response_freshness_property_based_recent_times() {
        ...
        for seconds_ago in [0, 30, 60, 120, 240, 299] { // Up to 4 min 59 sec ago
            let time = Utc::now() - Duration::seconds(seconds_ago);
            let date_header = time.format("%a, %d %b %Y %H:%M:%S GMT").to_string();
            
            let result = VisionLicenseManagerTestWrapper::check_response_freshness(&date_header);
            assert!(result.is_ok(), "Time {} seconds ago should be valid", seconds_ago);
        }
    }
    ...
    async fn test_check_response_freshness_property_based_stale_times() {
        ...
        for minutes_ago in [6, 10, 30, 60, 120] {
            ...
            let result = VisionLicenseManagerTestWrapper::check_response_freshness(&date_header);
            assert!(result.is_err(), "Time {} minutes ago should be stale", minutes_ago);
        }
    }
    ...
    async fn test_check_response_freshness_property_based_future_times() {
        ...
        for seconds_future in [1, 30, 59] { // Up to 59 seconds future
            let time = Utc::now() + Duration::seconds(seconds_future);
            let date_header = time.format("%a, %d %b %Y %H:%M:%S GMT").to_string();
            
            let result = VisionLicenseManagerTestWrapper::check_response_freshness(&date_header);
            assert!(result.is_ok(), "Time {} seconds in future should be valid", seconds_future);
        }
        ...
        for minutes_future in [2, 5, 10] {
            ...
            let result = VisionLicenseManagerTestWrapper::check_response_freshness(&date_header);
            assert!(result.is_err(), "Time {} minutes in future should be invalid", minutes_future);
        }
    }
    ...
    async fn test_verify_response_signature_malformed_headers_variants() {
        ...
        for (i, header) in malformed_headers.iter().enumerate() {
            let result = VisionLicenseManagerTestWrapper::verify_response_signature(
                header, date_header, response_body
            );
            assert!(result.is_err(), "Malformed header {} should return error: {}", i, header);
        }
    }
    ...
    async fn test_security_replay_attack_with_captured_response() {
        ...
        let date_header = captured_time.format("%a, %d %b %Y %H:%M:%S GMT").to_string();
        
        let result = VisionLicenseManagerTestWrapper::check_response_freshness(&date_header);
        
        assert!(result.is_err(), "Replayed response should be rejected");
        let error = result.unwrap_err().to_string();
        assert!(error.contains("replay attack"), "Error should mention replay attack");
    }
    ...
    async fn test_security_mitm_attack_modified_body() {
        ...
        let result = VisionLicenseManagerTestWrapper::verify_response_signature(
            &sig_header, &date_header, tampered_body
        );
        
        assert!(result.is_err(), "Modified response should fail signature verification");
        let error = result.unwrap_err().to_string();
        assert!(error.contains("MITM attack"), "Error should mention MITM attack");
    }
    ...
    async fn test_security_algorithm_downgrade_attack() {
        ...
        for algo in weak_algorithms {
            ...
            assert!(
                ...
                "Error should reject non-Ed25519 algorithm: {}", algo
            );
        }
    }
    ...
    async fn test_security_clock_tampering_attack() {
        ...
        let error = result.unwrap_err().to_string();
        assert!(error.contains("clock tampering"), "Error should mention clock tampering");
    }
    ...
    async fn test_security_truncated_signature_attack() {
        ...
        for sig in truncated_signatures {
            ...
            assert!(result.is_err(), "Truncated signature '{}' should be rejected", sig);
        }
    }
