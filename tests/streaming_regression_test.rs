...
fn test_sse_data_prefix_format() {
    ...
    assert_eq!(sse_line.matches("data:").count(), 1, "Should have exactly one 'data:' prefix");
...
fn test_sse_final_chunk_format() {
    ...
    assert_eq!(done_chunk.matches("data:").count(), 1, "Final chunk should have exactly one 'data:' prefix");
    assert!(done_chunk.starts_with("data: "), "Final chunk should start with 'data: '");
    assert!(done_chunk.contains("[DONE]"), "Final chunk should contain [DONE]");
    assert!(done_chunk.ends_with("\n\n"), "Final chunk should end with double newline");
}
...
fn test_sse_openai_compatibility() {
    ...
    for chunk in openai_compatible_chunks {
        ...
        assert_eq!(chunk.matches("data:").count(), 1, "Chunk should have exactly one 'data:' prefix: {}", chunk);

        // Each chunk should start with 'data: ' and end with '\n\n'
        assert!(chunk.starts_with("data: "), "Chunk should start with 'data: ': {}", chunk);
